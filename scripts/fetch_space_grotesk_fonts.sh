...
# Downloads Space Grotesk font files into public/fonts/space-grotesk.
# Provide either:
#   1) FONT_BASE_URL pointing to individual .woff2 files, or
#   2) FONT_ZIP_URL pointing to a zip archive containing the fonts.
#
# Examples:
#   FONT_BASE_URL="https://example.com/fonts/space-grotesk" \
#   ./scripts/fetch_space_grotesk_fonts.sh
#
#   FONT_ZIP_URL="https://example.com/space-grotesk.zip" \
#   ./scripts/fetch_space_grotesk_fonts.sh
...
TARGET_DIR="public/fonts/space-grotesk"
mkdir -p "$TARGET_DIR"
...
if [[ -n "${FONT_ZIP_URL:-}" ]]; then
  tmp_dir="$(mktemp -d)"
  zip_path="$tmp_dir/space-grotesk.zip"
